...
# --- Regularization helpers: penalize Gaussian mass outside the fit band [lo, hi] ---

from numpy import sqrt
SQRT_HALF_PI = np.sqrt(np.pi / 2.0)

def _band_leakage_penalty(flat_params, lo, hi):
    """Sum of Gaussian mass that lies outside [lo, hi] for each fitted peak.
    flat_params: [mu1, A1, sig1, mu2, A2, sig2, ...]
    """
    arr = np.asarray(flat_params, dtype=float).reshape(-1, 3)
    if arr.size == 0:
        return 0.0
    total = 0.0
    root2 = np.sqrt(2.0)
    for mu, A, sig in arr:
        if sig <= 0.0 or A <= 0.0:
            continue
        z_lo = (lo - mu) / (root2 * sig)
        z_hi = (hi - mu) / (root2 * sig)
        # area below lo + area above hi (constants don’t matter for optimization scale)
        mass_below = A * sig * SQRT_HALF_PI * (1.0 + erf(z_lo))
        mass_above = A * sig * SQRT_HALF_PI * (1.0 - erf(z_hi))
        total += max(mass_below, 0.0) + max(mass_above, 0.0)
    return max(total, 0.0)

def _residual_with_penalty(flat_params, freqs, y_flat, lam, lo, hi):
    """Per-bin residuals + 1 scalar residual => ||r||^2 = ||base||^2 + lam*penalty."""
    model_flat = gaussian_function(freqs, *flat_params)   # periodic model on flattened spectrum
    base = y_flat - model_flat
    if lam > 0.0:
        pen = _band_leakage_penalty(flat_params, lo, hi)
        return np.hstack([base, np.sqrt(lam * pen)])
    return base
...
class SpectralFitAlgorithm():
    ...
    def __init__(self, peak_width_limits=(0.5, 12.0), max_n_peaks=np.inf, min_peak_height=0.0,
                 ...
                 maxfev=5000, tol=0.00001, regularization_weight=0.0):  # ADD THIS PARAMETER
    ...
    def _fit_peak_guess(self, guess):
        ...
        lo_bound = [[peak[0] - 2 * self._cf_bound * peak[2], 0.0, self._gauss_std_limits[0]]
                    for peak in guess]
        hi_bound = [[peak[0] + 2 * self._cf_bound * peak[2], np.inf, self._gauss_std_limits[1]]
                    for peak in guess]
    
               # Clamp CF bounds to the analysis frequency range
        lo_bound = [[max(b[0], self.freq_range[0]), b[1], b[2]] for b in lo_bound]
        hi_bound = [[min(b[0], self.freq_range[1]), b[1], b[2]] for b in hi_bound]  # <- use hi_bound here
        
        # Ensure strict inequality: lb < ub for every parameter
        eps = 1e-9
        for lb, ub in zip(lo_bound, hi_bound):
            if lb[0] >= ub[0]:
                # nudge upper CF bound just above lower CF bound
                ub[0] = float(lb[0]) + eps
            if lb[1] >= ub[1]:
                ub[1] = float(lb[1]) + eps
            if lb[2] >= ub[2]:
        ...
        # Flatten bounds into the tuple format least_squares expects
        gaus_param_bounds = (
            tuple(item for sub in lo_bound for item in sub),
            tuple(item for sub in hi_bound for item in sub),
        )
        ...
        try:
            lambda_reg = float(getattr(self, "regularization_weight", 0.0) or 0.0)
            
            res = least_squares(
                _residual_with_penalty,
                guess,
                bounds=gaus_param_bounds,
                args=(self.freqs, self._spectrum_flat, lambda_reg, self.freq_range[0], self.freq_range[1]),
                method='trf',
                jac='2-point',
                max_nfev=self._maxfev,
                ftol=self._tol, xtol=self._tol, gtol=self._tol,
            )
                
    
            gaussian_params = res.x
    
            # Optional debug hooks to verify λ is actually seen in this run
            try:
                self._last_reglambda = lambda_reg
                self._last_penalty_value = _negfreq_penalty(gaussian_params)
                self._last_residual_cost = res.cost
            except Exception:
                pass
        ...
        # Re-organize params into 2d matrix (rows of 3)
