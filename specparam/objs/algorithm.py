...
def _negfreq_penalty(flat_params):
    """Sum of Gaussian mass that lies at f < 0 for each fitted peak.
    ...
    for mu, A, sig in arr:
        if sig <= 0:
            continue
        z = (0.0 - mu) / (np.sqrt(2.0) * sig)
        total += A * sig * np.sqrt(np.pi / 2.0) * (1.0 + erf(z))
    return max(total, 0.0)  # numerical guard

def _residual_with_penalty(flat_params, freqs, y_flat, lam):
    """Per-bin residuals + 1 scalar residual so ||r||^2 = ||base||^2 + lam * penalty."""
    model_flat = gaussian_function(freqs, *flat_params)   # periodic model on flattened spectrum
    base = y_flat - model_flat
    if lam > 0.0:
        pen = _negfreq_penalty(flat_params)
        return np.hstack([base, np.sqrt(lam * pen)])
    return base
# --- End regularization helpers ---
class SpectralFitAlgorithm():
    ...
    def _fit_peak_guess(self, guess):
        ...
        # Keep CF bounds within frequency range
        lo_bound = [b if b[0] > self.freq_range[0] else [self.freq_range[0], *b[1:]] for b in lo_bound]
        hi_bound = [b if b[0] < self.freq_range[1] else [self.freq_range[1], *b[1:]] for b in hi_bound]
    
        # Flatten bounds into the tuple format curve_fit/least_squares expects
        gaus_param_bounds = (
        ...
        try:
            # λ (default 0.0 means 'off'; identical behavior to upstream)
            lambda_reg = getattr(self, "regularization_weight", 0.0)
    
            # Least-squares over the flattened spectrum residuals + 1 penalty residual
            res = least_squares(
                _residual_with_penalty,
                guess,
                bounds=gaus_param_bounds,
                args=(self.freqs, self._spectrum_flat, lambda_reg),
                method='trf',
                jac='2-point',               # keep it simple; optional to add analytic Jacobian later
